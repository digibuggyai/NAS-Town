...
function Login({ onLogin }) {
  ...
  return (
    <div className="grid min-h-screen place-items-center px-4">
      <form onSubmit={submit} className="glass w-full max-w-sm rounded-[1.75rem] p-7">
        <Logo />
        ...
        <label className="mt-3 block">
          ...
          <span className="relative block">
            ...
            <button
              ...
              className="absolute top-1/2 right-2 grid size-8 -translate-y-1/2 place-items-center rounded-full text-white/50 transition-colors hover:bg-white/10 hover:text-white"
            >
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      <header className="glass flex flex-wrap items-center justify-between gap-4 rounded-full py-2 pr-2 pl-5">
        <div className="flex items-center gap-4">
          <Logo />
          <span className="hidden rounded-full bg-white/10 px-2.5 py-1 text-[0.65rem] tracking-wide text-muted uppercase sm:inline">Staff · {user.role}</span>
        </div>
        <nav className="order-3 flex w-full gap-1 overflow-x-auto sm:order-none sm:w-auto">
          {tabs.map(([label, to]) => (
            <NavLink key={to} to={to} end className={({ isActive }) => `rounded-full px-3 py-1.5 text-[0.8rem] whitespace-nowrap transition-colors ${isActive ? 'bg-white text-black' : 'text-white/70 hover:bg-white/[0.07] hover:text-white'}`}>
              {label}
            </NavLink>
          ))}
        </nav>
        <div className="flex items-center gap-2">
          ...
          <button onClick={onLogout} className="btn btn-glass !p-2" aria-label="Sign out"><LogOut className="size-4" /></button>
        </div>
      </header>
  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <button onClick={() => api.downloadPriceSheet().catch((e) => alert(e.message))} className="btn btn-glass !py-2 !text-xs">
      ...
      <div className="glass mt-4 rounded-[1.5rem] p-4 sm:p-5">
        {tab === 'settings' ? (
  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-3">
        ...
        {data.map((e) => (
          <details key={e.id} className="glass rounded-2xl">
            <summary className="flex cursor-pointer list-none flex-wrap items-center gap-x-4 gap-y-1 p-4 text-sm">
              <span className="rounded-full bg-white/10 px-2 py-0.5 text-xs capitalize">{e.type}</span>
            ...
            <div className="border-t border-line p-4 text-sm">
              ...
              {e.payload?.summary && <pre className="mt-3 overflow-x-auto rounded-xl bg-white/[0.04] p-3 font-mono text-xs whitespace-pre-wrap">{e.payload.summary}</pre>}
            </div>
          </details>
        ))}
      </div>
    ...
    <section className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div>
        ...
        <ul className="mt-6 grid gap-2">
          {data?.map((u) => (
            <li key={u.id} className="glass flex items-center gap-3 rounded-2xl px-4 py-3 text-sm">
              <span className="rounded-full bg-white/10 px-2 py-0.5 text-xs capitalize">{u.role}</span>
              <span className="flex-1">{u.email}{u.name ? <span className="text-muted"> · {u.name}</span> : null}</span>
              {u.id !== me.id && <button onClick={() => remove(u)} className="text-xs text-white/50 hover:text-red-300">Remove</button>}
            </li>
          ))}
        </ul>
      </div>
      <form onSubmit={add} className="glass h-fit rounded-[1.5rem] p-5">
