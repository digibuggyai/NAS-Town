...
// Stylized NAS render used until real product photography is added.
// `bays` changes the number of drive sleds; `glow` adds the ambient under-light.
export default function NasVisual({ bays = 4, glow = true, className = '' }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} 340`} className={className} role="img" aria-label={`${bays}-bay NAS`}>
      ...
      {glow && <ellipse cx={w / 2} cy="318" rx={w * 0.42} ry="22" fill={`url(#glow-${id})`} />}
      ...
      {Array.from({ length: cols }).map((_, i) => {
        ...
        return (
          <g key={i}>
            ...
            <circle cx={x + bayW / 2} cy="232" r="3.2" fill="var(--color-accent)" className="animate-blink" style={{ animationDelay: `${i * 0.35}s` }} />
          </g>
        );
      })}

      <circle cx="52" cy="276" r="7" fill="none" stroke="#fff" strokeOpacity="0.4" />
      <circle cx="52" cy="276" r="2.2" fill="var(--color-accent)" />
