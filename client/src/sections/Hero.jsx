...
const strip = ['Synology DSM', 'QNAP QTS', 'UGREEN UGOS', 'Asustor ADM', 'RAID 1 · 5 · 6 · 10', 'SHR', '2.5GbE & 10GbE', 'NVMe caching', 'Snapshots', 'Hybrid backup', 'Remote access', 'Surveillance'];
...
export default function Hero() {
  ...
  useGSAP(() => {
    ...
    tl.from('.hero-eyebrow', { autoAlpha: 0, y: 14, duration: 1 })
      ...
      .from('.hero-links', { autoAlpha: 0, duration: 1 }, 0.9)
  ...
  return (
    <section ref={root} className="relative pt-32 pb-8 text-center">
      <div className="hero-copy mx-auto flex max-w-7xl flex-col items-center px-4 sm:px-6">
        <p className="hero-eyebrow eyebrow">Store · Share · Protect · Grow</p>
        <SplitHeading
          as="h1"
          immediate
          delay={0.45}
          text="Your Data Deserves a *Home.*"
          className="mt-6 text-[2.6rem] sm:text-6xl lg:text-7xl"
        />
        <p className="hero-sub mx-auto mt-6 max-w-md text-base text-muted sm:text-lg">
        ...
        <div className="hero-links mt-7 flex flex-wrap justify-center gap-x-5 gap-y-2 text-[0.8rem] text-white/50">
          <Link to="/solutions/creators" className="transition-colors hover:text-white">For Creators</Link>
          <span aria-hidden className="text-white/20">/</span>
          <Link to="/solutions/business" className="transition-colors hover:text-white">For Business</Link>
          <span aria-hidden className="text-white/20">/</span>
          <Link to="/solutions/home" className="transition-colors hover:text-white">For Home</Link>
        </div>
      </div>
